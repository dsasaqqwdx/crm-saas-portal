...
exports.getSummary = async (req, res) => {
    try {
        ...
        // 👉 ADMIN VIEW (ONLY when NOT self)
        if (['company_admin', 'super_admin', 'software_owner'].includes(role) && !isSelf) {
                ...
                SELECT 
                    (SELECT status FROM public.attendance 
                     WHERE employee_id = (
                        ...
                        WHERE email = (
                            SELECT email FROM public.users WHERE user_id = $1
                        )
                     ) 
                     AND attendance_date = CURRENT_DATE LIMIT 1) as "attendanceToday",

                    (SELECT COALESCE(SUM(remaining_leaves), 0)::int 
                    ...
                    (SELECT COUNT(*)::int 
                     ...
                     WHERE company_id = $2 
                     AND holiday_date >= CURRENT_DATE) as "upcomingHolidays",
            ...
            // ✅ FIX: convert "present" → "Marked"
            data.attendanceToday =
                data.attendanceToday &&
                data.attendanceToday.toLowerCase() === "present"
                    ? "Marked"
                    : "Not Marked";
