...
//   else cb(new Error("File type not allowed. Allowed: images, PDF, Word, Excel, TXT"), false);
// };

// const upload = multer({ storage, fileFilter, limits: { fileSize: 5 * 1024 * 1024 } });

// // POST — upload file for a ticket
...
//       "SELECT * FROM ticket_attachments WHERE attachment_id = $1", [attachment_id]
//     );
...
router.post("/:ticket_id", auth, upload.single("file"), async (req, res) => {
  try {
    ...
    if (role === "company_admin" || role === "super_admin" || role === "software_owner") {
      // Admin uploaded — notify the employee
