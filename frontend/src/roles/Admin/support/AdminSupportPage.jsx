...
const updated = allTickets.find(t => t.ticket_id === selectedTicket.ticket_id);
      if (updated) setSelectedTicket(updated); } catch (err) {
...
fontWeight: 500, boxShadow: "0 4px 12px rgba(0,0,0,0.15)"            
 ...
 className={`btn btn-sm fw-semibold ${filterStatus === s ? "btn-primary" : "btn-outline-secondary"}`}>
  ...
  const sc = STATUS_COLORS[ticket.status] || STATUS_COLORS.open;  const isSelected = selectedTicket?.ticket_id === ticket.ticket_id;
  const convCount = parseJSON(ticket.conversation).length;
                  return (        <div key={ticket.ticket_id}
  ...
  <div className="d-flex justify-content-between align-items-start mb-1">     <span style={{ fontWeight: 600, fontSize: 13, color: "#1e293b" }}>
      {ticket.user_name || "Unknown"} {ticket.status === "open" && (
 ...
 {parseJSON(selectedTicket.conversation).map((msg, idx) => (
                      <div key={`conv-${idx}`} style={{ display: "flex", justifyContent: "flex-start" }}>
                        <div style={{
               maxWidth: "80%", padding: "8px 12px",
         borderRadius: "12px 12px 12px 3px",        background: "#e0e7ff",
               color: "#1e293b",
                          fontSize: 13, lineHeight: 1.5,
            boxShadow: "0 1px 3px rgba(0,0,0,0.08)",
                        }}>
