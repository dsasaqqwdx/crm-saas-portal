...
const NotificationContext = createContext(null);const TYPE_ICONS = {
  new_ticket:       "",
  admin_reply:      "",
  employee_message: "",
  file_uploaded:    "",
...
const dropdownRef = useRef(null);
  const getToken = () => localStorage.getItem("token");const getHeaders = () => ({ "x-auth-token": getToken() });
...
lastCountRef.current = 0;
  ...
  return (
    <NotificationContext.Provider value={{ notifications, unreadCount, markAsRead, markAllRead, clearAll, dropdownOpen, setDropdownOpen, dropdownRef, fetchNotifications, TYPE_ICONS, TYPE_COLORS, TYPE_LABELS, formatTime,
    }}>
  {children} <div style={{
 position: "fixed", top: 16, right: 16, zIndex: 99999,
        display: "flex", flexDirection: "column", gap: 10,
        pointerEvents: "none",
      }}>  {toasts.map(toast => (    <div key={toast.toastId}
    style={{
...
export function NotificationBell() {
  ...
  cursor: "pointer", position: "relative",    display: "flex", alignItems: "center", justifyContent: "center",
        ...
        title="Notifications"
      >
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">   <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" />
...
<div style={{
   ...
   <div style={{ fontWeight: 700, fontSize: 15, color: "#1e293b", display: "flex", alignItems: "center", gap: 8 }}>
    ...
    <div style={{ textAlign: "center", padding: "32px 16px", color: "#94a3b8" }}>
              ...
              <div key={n.notification_id}      onClick={() => markAsRead(n.notification_id)}
                style={{
           padding: "11px 16px",
