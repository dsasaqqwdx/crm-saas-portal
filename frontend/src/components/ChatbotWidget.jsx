...
const messagesEndRef = useRef(null);
 ...
 if (!ticket) return;
  ...
  const handleSend = async (text) => {
    ...
    setInput("");  setShowQuestions(false);
...
setLoading(true);
  ...
  setLoading(false);
 addMessage("assistant", "Sure! I'll connect you with our support team. Please click **Contact Support Team** below to create a ticket. 🎫"); setShowContactOption(true);
      return;
    }
   ...
   messages: history
      ...
      const data = await response.json();   const aiReply = data.content?.[0]?.text || "I'm not sure about that. Please contact support for help.";
addMessage("assistant", aiReply);
  ...
  const currentName = getUserName();
    try {
      const chatHistory = messages.map(m => ({ role: m.role === "admin" ? "assistant" : m.role,
  content: m.content,
...
messages: chatHistory,
 ...
 if (!ticketId) return;
    ...
    try {
const res = await axios.get(`${API}/api/support/my`, { headers: getHeaders() });
const ticket = (res.data.data || []).find(t => t.ticket_id === ticketId);     if (!ticket) { addMessage("assistant", "Couldn't find your ticket. Please try again."); return; }
      let conversation = ticket.conversation || [];
   if (typeof conversation === "string") {
        try { conversation = JSON.parse(conversation); } catch { conversation = []; }
      }
  ...
  return;  }
   const newReplies = conversation.filter((_, idx) => idx >= seenReplies.length);
      if (newReplies.length > 0) {   setSeenReplies(conversation);
 newReplies.forEach(reply => {
          addMessage("admin", ` **${reply.sender || "Admin"} replied:** ${reply.content}`);
        });  } else {
...
<div style={{ maxWidth: "85%" }}>
 ...
 border: style.border || "none",
     ...
     dangerouslySetInnerHTML={{ __html: renderContent(msg.content) }}
      ...
      <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 3, textAlign: style.align === "flex-end" ? "right" : "left" }}>
            ...
            {loading && (
              <div style={{ display: "flex", gap: 8 }}>      <div style={{ padding: "10px 14px", borderRadius: "16px 16px 16px 4px", background: "#f1f5f9", display: "flex", gap: 4, alignItems: "center" }}>
 ...
 {showContactOption && !loading && (
  ...
  {QUICK_QUESTIONS.map((q, i) => (
   <button key={i} onClick={() => handleSend(q)}       style={{ background: "#e0e7ff", color: "#4f46e5", border: "none", borderRadius: 20, padding: "4px 10px", fontSize: 11.5, cursor: "pointer", fontWeight: 500 }}
