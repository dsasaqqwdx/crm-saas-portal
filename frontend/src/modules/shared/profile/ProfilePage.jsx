...
// // Decides which backend endpoint to use based on role
// const getProfileEndpoint = (role, activeRole) => {
//   // company_admin in "self" view → use employee profile endpoint
//   // pure employee → use employee profile endpoint
//   // anything else → employee endpoint as fallback
//   const isAdminInSelfMode = role === "company_admin" && activeRole === "self";
//   const isEmployee = role === "employee";

//   if (isAdminInSelfMode || isEmployee) {
//     return `${API_BASE}/api/employees/profile`;
//   }
//   // fallback
//   return `${API_BASE}/api/employees/profile`;
// };
...
//   const profileEndpoint = getProfileEndpoint(role, activeRole);
...
//   const deleteAvatar = async () => {
//     try {
//       // derive avatar DELETE endpoint from profile endpoint base
//       const avatarEndpoint = profileEndpoint.replace("/profile", "/avatar");
//       await axios.delete(avatarEndpoint, { headers });
//       setAvatarPreview(null);
//       setAvatarFile(null);
//       setAvatarError(false);
...
//     setPwSaving(true);
//     try {
//       const pwEndpoint = profileEndpoint.replace("/profile", "/change-password");
//       await axios.put(
//         pwEndpoint,
...
//     boxShadow: "0 4px 14px rgba(91,94,244,0.28)",
//   };
...
//     gap: "7px",
//     boxShadow: "0 4px 16px rgba(17,19,48,0.2)",
//     transition: "all 0.18s",
//   };
...
//             >
//               {role === "company_admin" ? "Self View" : "Employee"} • Profile
//             </div>
...
//           // ── Profile not found (employee record doesn't exist yet for this admin) ──
//           ) : notFound ? (
//             <div style={{ ...card, textAlign: "center", padding: "60px 40px" }}>
//               <div style={{ fontSize: "48px", marginBottom: "16px" }}>👤</div>
//               <h4 style={{ color: "#111330", fontWeight: 700 }}>No Employee Profile Found</h4>
//               <p style={{ color: "#8e91b8", maxWidth: "400px", margin: "8px auto 0" }}>
//                 Your admin account doesn't have an employee profile linked yet. Please contact your system administrator to create one, or switch to the Manager view.
//               </p>
//             </div>
...
//                             type="button"
//                             onClick={toggle}
//                             style={{ position: "absolute", right: "11px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#8e91b8", padding: "3px" }}
//                           >
//                             {show
...
const ProfilePage = () => {
  ...
  const [avatarError, setAvatarError] = useState(false);
  const [toast, setToast]             = useState(null);
  // isNewProfile = true means no record existed yet → PUT will INSERT
  ...
  // Always use the employee profile endpoint.
  // The backend GET already falls back: employee_profiles → admin_profiles → users
  // The backend PUT already upserts: UPDATE if exists, INSERT if not
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      if (status === 404) {
        // No employee_profiles row yet for this user.
        // Show an empty form — PUT /profile will INSERT (upsert) on save.
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      // PUT upserts: updates existing row OR inserts new one
      await axios.put(profileEndpoint, formData, { headers });

      showToast(isNewProfile ? "Profile created successfully!" : "Profile updated successfully");
      setAvatarFile(null);
      setIsNewProfile(false);
      loadProfile(); // reload to get server-confirmed avatar URL
    } catch (err) {
      showToast(err.response?.data?.message || "Error updating profile", "error");
    } finally {
      setSaving(false);
